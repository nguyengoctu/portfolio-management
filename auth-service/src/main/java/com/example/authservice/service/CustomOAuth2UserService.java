...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private User processGitHubUser(OAuth2User oauth2User) {
        ...
        // If email is null, try to get it from GitHub emails API (for authenticated requests)
        if (email == null || email.isEmpty()) {
            logger.warn("GitHub email is null for user: {}. User needs to make email public in GitHub settings.", login);
            // For now, we'll proceed without email - user can add it later or make it public in GitHub
        }
        ...
        if (existingUser != null) {
            ...
            logger.info("Updated existing GitHub user: {}", existingUser.getName());
            return userRepository.save(existingUser);
        }
        
        // Check if user exists with same email but different provider (only if email is available)
        if (email != null && !email.isEmpty()) {
